...
# Instância CPU. AUTO-TERMINA. Saídas: s3://$BUCKET/runs/b3/.
set -euo pipefail
exec > /var/log/tcc-b3.log 2>&1

BUCKET=video-analytics-store
REGION=us-east-1
echo "=== TCC B3 job (Pegasus vídeo-nativo): $(date -u) ==="
...
mkdir -p clips
DUR=$(ffprobe -v error -show_entries format=duration -of csv=p=0 main_5fps.mp4 | cut -d. -f1)
N=$(( (DUR + 59) / 60 ))
for i in $(seq 0 $((N - 1))); do
  ffmpeg -v error -ss $((i * 60)) -t 60 -i main_5fps.mp4 -c copy "clips/c$(printf %03d $i).mp4" -y
done
aws s3 sync clips "s3://$BUCKET/pipeline-b/b3_clips/" --region $REGION --only-show-errors
echo "clipes: $N"
...
t_all = time.time()
for i in range(N):
    uri = f"s3://video-analytics-store/pipeline-b/b3_clips/c{i:03d}.mp4"
    t0 = time.time()
    try:
...
aws s3 cp b3_results.json s3://$BUCKET/runs/b3/b3_results.json --region $REGION --only-show-errors
aws s3 cp /var/log/tcc-b3.log s3://$BUCKET/runs/b3/job_log.txt --region $REGION --only-show-errors || true
