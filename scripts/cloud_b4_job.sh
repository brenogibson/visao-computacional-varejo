...
# Instância CPU (inferência no Bedrock). AUTO-TERMINA. Saídas: s3://$BUCKET/runs/b4/.
set -euo pipefail
exec > /var/log/tcc-b2.log 2>&1

BUCKET=video-analytics-store
REGION=us-east-1
echo "=== TCC B4 job (escalabilidade: gemma-31b + luna no video de 67min): $(date -u) ==="
...
aws s3 cp s3://$BUCKET/videos/derived/scale_5fps.mp4 scale_5fps.mp4 --region $REGION --only-show-errors
...
aws s3 cp b2_out.zip s3://$BUCKET/runs/b4/b2_out.zip --region $REGION --only-show-errors
aws s3 cp b2_summaries.json s3://$BUCKET/runs/b4/b2_summaries.json --region $REGION --only-show-errors
aws s3 cp /var/log/tcc-b2.log s3://$BUCKET/runs/b4/job_log.txt --region $REGION --only-show-errors || true
